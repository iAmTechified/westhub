...
class GoogleSheetsService
{
    ...
    public function appendJoinRequestRow(array $payload): void
    {
        ...
        $response = Http::withToken($this->accessToken())
            ->acceptJson()
            ->post($this->valuesUrl($this->sheetName().'!A:P').':append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS', [
                'values' => [$this->rowValues($payload)],
            ]);
    ...
    protected function rowValues(array $payload): array
    {
        return [
            (string) data_get($payload, 'westhub_id', ''),
            (string) data_get($payload, 'submitted_at', ''),
            (string) data_get($payload, 'full_name', ''),
            (string) data_get($payload, 'email', ''),
            (string) data_get($payload, 'phone', ''),
            (string) data_get($payload, 'applicant_type', ''),
            (string) data_get($payload, 'position_profession', ''),
            (string) data_get($payload, 'date_available', ''),
            (string) data_get($payload, 'sheet_stage', ''),
            (string) data_get($payload, 'westhub_decision', ''),
            (string) data_get($payload, 'owner', ''),
            (string) data_get($payload, 'notes', ''),
            (string) data_get($payload, 'admin_url', ''),
            (string) data_get($payload, 'resume_url', ''),
            (string) data_get($payload, 'signature_url', ''),
            implode("\n", data_get($payload, 'document_urls', [])),
        ];
    }
