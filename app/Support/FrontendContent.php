...
class FrontendContent
{
    ...
    public static function galleryImages(?int $limit = null): Collection
    {
        try {
            ...
            // 2. Fetch Design Fallbacks
            $fallbackImages = self::fallbackGalleryImages();

            // 3. Merge: Database images first (fresh content), then design fallbacks.
            // We deduplicate by title slug to ensure that if an image is in the DB, 
            // the DB version (with user customizations) replaces the static fallback.
            $allImages = $dbImages->concat($fallbackImages)
                ->unique(fn (array $image) => \Illuminate\Support\Str::slug($image['title']))
                ->values();

            if ($limit) {
                return $allImages->take($limit);
            }
    ...
    private static function galleryItemViewData(GalleryItem $item, int $index, ?object $media): ?array
    {
        ...
        if ($media) {
            // BRUTE FORCE FIX: If we have a media record in the DB, 
            // we ALWAYS try to show it on the root app. 
            // We do NOT check if the file exists locally.
    ...
    private static function mediaUrl(object $media): string
    {
        ...
        // Force use of the 'public' disk URL logic to respect PUBLIC_STORAGE_URL
